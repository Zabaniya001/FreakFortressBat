...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
    ...
    OnHiddenPossibility = new GlobalForward("FF2_OnSpecialSelectedForHidden", ET_Hook, Param_Cell, Param_CellByRef, Param_String, Param_Cell);  //Boss, character index, character name, preset
...
public Action OnTakeDamage(int client, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if(IsBoss(attacker) && IsValidClient(client))
    {
        ...
        else if(boss != -1)
        {
            ...
            if(bIsBackstab)
            {
                damage = BossHealthMax[boss] * BossLivesMax[boss] * (0.12 - Stabbed[boss] / 90) / 3;
                ...
                if(GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") == 225)
                {
                    damage = 10.0;
                    int iCurrentHP = GetClientHealth(attacker);
                    SetEntityHealth(attacker, iCurrentHP + 3);
            ...
            if(attacker <= MaxClients)
            {
                ...
                switch(index)
                {
                    ...
                    case 44:    //Sandman
                    {
                        if(cvarEnableSandmanStun.BoolValue)
                        {
                            ...
                            {
                                TF2_StunPlayer(client, 1.0, 0.0, TF_STUNFLAG_SLOWDOWN, attacker); 
                            }
                            else if (dist >= 256.0 && dist < 512.0) 
                            {
                                TF2_StunPlayer(client, 2.0, 0.0, TF_STUNFLAG_SLOWDOWN, attacker); 
                            }
                    ...
                    case 225: // YER
                    {
                        if(GetClientHealth(attacker) <= GetEntProp(attacker, Prop_Data, "m_iMaxHealth"))
                        {
                            int iCurrentHP = GetClientHealth(attacker);
                            SetEntityHealth(attacker, iCurrentHP + 1);
                        }
                    }
                }
                ...
                if(bIsBackstab)
                {
                    ...
                    if(GetEntProp(weapon, Prop_Send, "m_iItemDefinitionIndex") == 225)
                    {
                        damage = 5.0;
                        int iCurrentHP = GetClientHealth(attacker);
                        SetEntityHealth(attacker, iCurrentHP + 2);
...
public bool PickCharacter(int boss, int companion)
{
    ...
    // Hidden Bosses Forward
    Action nothing;
    Call_StartForward(OnHiddenPossibility);
    Call_PushCell(companion);
    Call_PushCellRef(characterIndex);
    Call_PushStringEx(newName, sizeof(newName), SM_PARAM_STRING_UTF8 | SM_PARAM_STRING_COPY, SM_PARAM_COPYBACK);
    Call_PushCell(false);  //Not preset
    Call_Finish(nothing);
    if(nothing == Plugin_Changed)
    {
        if(newName[0])
        {
            int foundExactMatch = -1;
